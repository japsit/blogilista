...
beforeEach(async () => {
  ...
  const firstUser = new User({ username: 'testuser', passwordHash })
  await firstUser.save()

  // Luodaan käyttäjätiedot tokenia varten
  const userForToken = {
    username: firstUser.username,
    id: firstUser._id,
  }

  // Luo token tällä käyttäjätiedolla
  token = jwt.sign(userForToken, process.env.SECRET, { expiresIn: '1h' })

  // Alusta blogit ja liitä ne firstUser-käyttäjään

  let blogObject = new Blog({ ...initialBlogs[0], user: firstUser._id })
  const savedBlog = await blogObject.save()
  firstUser.blogs = firstUser.blogs.concat(savedBlog._id)
  blogObject = new Blog({ ...initialBlogs[1], user: firstUser._id })
  const savedBlogTwo = await blogObject.save()
  firstUser.blogs = firstUser.blogs.concat(savedBlogTwo._id)
  await firstUser.save()
})
...
test('Delete blog', async() => {
  ...
  // Testaa, ettei voidan poistaa toisten blogeja, jotka eivät kuulu käyttäjäle
